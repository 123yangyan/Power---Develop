...
- **约定** 数据来自 `storage.hr`；图表可降采样，存储层全量。
- **验收** 展示样本数/平均/最低/最高；折线图最近样本。
...
- **入口** 设备页连点版本 7 次 → 开发者卡片 → 运行日志/storage 看板/PPI 上传日志
...
- **入口** `MindBodyApplication.onCreate()` → `BleSchedulerWorker.scheduleNext`；设备页「夜间自动断联」卡片
...
- **入口** 连接成功后自动弹出 CDM chooser；「后台保活」卡片可手动重试
...
- **约定** Polar SDK 仍负责 BLE；CDM 仅 OS 层；用户取消 chooser 降级现有保活。
- **验收** Android 8+ 可选伴随设备；设备页显示「已关联」；日志 `CDM verified` 或警告。

> 2026-06-26 CDM 伴随设备关联 (#cdm-companion-keepalive)

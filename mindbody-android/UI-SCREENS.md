...
MindBody Android 采用 **100% Jetpack Compose** 实现，主流程为 **单 Activity + NavHost**，无 Fragment、无 XML 导航图。用户日常在 `MainActivity` 内通过底部 **5 Tab** 切换；定时心情探查则通过独立 `MoodCheckInActivity` 以全屏 Bottom Sheet 形式出现。
...
flowchart TB
    ...
    subgraph main [MainActivity 主壳]
        NAV[FloatingIslandNav 底部 5 Tab]
        NAV --> HR[心率]
        NAV --> PS[状态]
        NAV --> MR[记录]
        NAV --> TL[时间]
...
| 底部 Tab 页 | 5 | 见上表 |
| 栈内子页 | 6 | 反馈历史、时间线、FTU、运行日志、storage 看板、PPI 上传日志 |
...
| 模式 | 说明 | 典型使用页 |
|------|------|------------|
| **Head / Middle / Bottom** | 上方 20% 大指标 Hero，中间 50% 叙事卡，下方 30% 数据网格 | 心率、状态 |
...
| **MicroGrid** | 2 列统计格，支持单位与小色块 | 今日 HR、HRV 六格 + 实时传感器读数 |
...
**页面定位**：Polar Loop 实时心率信号的「第一视窗」，聚焦 BPM、连接状态与今日统计。

#### 界面组成

| 区块 | 用户看到的内容 | 功能点 |
|------|----------------|--------|
| **电量** | 左上角「电量 N%」（已连接时；≤20% 红色） | 来自 Polar BLE 电量回调 |
| **Hero 区** | 大号 BPM 数字（未连接显示 `--`）· **唯一** BLE 状态副文案 · 环形进度 | 实时心率；进度环随 BPM/200 变化 |
...
把 Polar Loop 的客观信号变成「**可读的身体陪伴日记**」，而非裸数字仪表盘。实时皮肤温度已移至状态 Tab「实时传感器」卡片。

#### 连接状态文案（Hero 副标题，全页唯一一套）
...
| **身心分析指标** | 基线完成后：HRV 六格（副交感调节、整体心率变异等），数值按区间五档变色 | 过低紫/蓝，正常绿，过高橙/红 |
| **实时传感器** | 始终显示：皮肤温度、合加速度、心跳间期、皮肤接触 | 本地 BLE 传感器流；皮温 33–36.5°C 为舒适区间 |
...
| 区块 | 内容 |
|------|------|
| TopBar | 标题「时间线」+ 返回 + 日历图标（预留） |
| 周历条 | 当周 7 天横排，选中日 Teal 圆圈高亮 |
| 日汇总 | 步数 · 平均心率 · 焦虑事件次数 · 睡眠（暂 `—`） |
| 事件列表 | 纵向时间轴：训练（时长/HR）· 心情（角色/日记/HR）· 身心反馈（焦虑指数/LLM 摘要） |

**用户价值**：把分散的训练、心情与 AI 反馈合成「一天的身体日记」，便于回顾全天身心变化节奏。
...
| 时间线 | `ui/timeline/TimelineScreen.kt` · `TimelineViewModel.kt` |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-06-26 | 0.4.4 | 新增时间线栈内页（F-P2-014）；历史 Tab 页眉入口 |

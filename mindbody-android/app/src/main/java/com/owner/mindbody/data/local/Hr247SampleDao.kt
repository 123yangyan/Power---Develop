...
interface Hr247SampleDao : SyncableDao<Hr247SampleEntity> {
    ...
    override suspend fun getUnsynced(limit: Int): List<Hr247SampleEntity>

    @Query(
        """
        UPDATE hr_247_samples
    ...
    @Query(
        """
        DELETE FROM hr_247_samples WHERE id IN (
            SELECT id FROM hr_247_samples

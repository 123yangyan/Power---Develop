...
@Dao
interface Ppi247SampleDao : SyncableDao<Ppi247SampleEntity> {

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insertAll(samples: List<Ppi247SampleEntity>)

    @Query(
        """
        SELECT * FROM ppi_247_samples
    ...
    override suspend fun getUnsynced(limit: Int): List<Ppi247SampleEntity>

    @Query(
        """
        UPDATE ppi_247_samples
    ...
    @Query(
        """
        DELETE FROM ppi_247_samples WHERE id IN (
            SELECT id FROM ppi_247_samples
